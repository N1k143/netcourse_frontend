<template>
  <div>
    ...
    <div v-else-if="!quizzes || quizzes.length === 0" class="bg-slate-900 border border-emerald-500/30 rounded-xl p-4 sm:p-8">
      <div class="text-center">
        <div class="text-slate-400 font-mono text-base sm:text-lg mb-4">
          // Тест не найден
        </div>
        <div class="text-slate-500 font-mono text-xs sm:text-sm mb-6">
    ...
    <div v-else-if="showResults && results" class="bg-slate-900 border border-emerald-500/30 rounded-xl p-3 sm:p-8">
          ...
          <div class="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-2 flex-wrap">
                <span class="text-emerald-400 font-mono text-xs sm:text-sm font-bold">
                  Вопрос {{ index + 1 }}:
                </span>
                <span :class="[
      ...
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
        ...
        <button
          @click="emit('close')"
          class="px-4 py-2.5 sm:px-6 sm:py-3 bg-gradient-to-r from-emerald-600 to-emerald-500 text-slate-950 rounded-lg font-mono text-sm sm:text-base hover:from-emerald-500 hover:to-emerald-400 transition-all hover:shadow-lg hover:shadow-emerald-500/50"
        >
          Вернуться к уроку
        </button>
      </div>
      ...
      <div v-if="currentQuestion" class="mb-6 sm:mb-8">
        <div class="bg-slate-800 rounded-xl p-3 sm:p-6 mb-4 sm:mb-6">
          <div class="flex items-start gap-2 sm:gap-3 mb-3 sm:mb-4">
            ...
            <p class="text-white text-sm sm:text-lg flex-1 break-words">
...
// Lifecycle
onMounted(() => {
  if (props.courseId && props.sectionId && props.lessonId) {
    loadQuizzes()
  }
})
